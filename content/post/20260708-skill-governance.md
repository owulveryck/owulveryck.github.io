...
draft: true
summary: "Skills are the distribution unit of the agentic workforce: they package reusable workflows and make them invocable by any agent in the organization. But today's package managers are capability-blind — they version and distribute, but they cannot validate governance policies. This article extends the Platform Planning Gateway proof of concept with a skill governance linter: a deterministic OPA/Rego policy engine that validates every skill before it enters the distribution chain, classifying it by security tier and enforcing the dual-representation artifact pattern that already governs agent plans."
...
That semantic nature is precisely why governance matters. A skill that instructs the agent to modify files without going through `lock_in_plan` bypasses the execution governance entirely. (`lock_in_plan` is the deterministic gate at the center of the [previous article's Platform Planning Gateway](/2026/07/07/amplified-agentic-loop/): before touching any file, the agent must submit its plan as a structured JSON contract to a gateway that runs OPA/Rego policies — non-conforming plans are rejected with semantic violations; conforming ones receive a signed capability ticket that scoped tools verify in-tool.) A skill distributed without a version cannot be audited or rolled back. A skill with a 10-character description is invisible in `/help`. These are not implementation bugs — they are governance gaps.
...
Tier classification is deliberately binary and based on keywords in the skill body — not on intent. A skill that *could* be safe but mentions `Bash` is tier 2 regardless, for the same reason the plan linter is not an LLM: determinism and reproducibility take priority over nuance.
...
When a consumer installs a skill from the registry — using `apm install` or equivalent — the PPG re-runs the validation. Consumers always install from the registry, never directly from skill repositories; this is the trust boundary the registry provides. Revalidation at install time closes the tamper window: a skill that passed validation at publish time might be modified in the registry, or the governance policies might have been tightened since publication. Install-time revalidation is the last structural check before the skill enters a project's `.claude/skills/`.
